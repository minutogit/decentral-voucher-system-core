...
Die reine Erkennung eines Konflikts reicht nicht aus. Die `voucher_core`-Bibliothek muss eine klare Strategie zur Handhabung implementieren. Die Reaktion des Wallets auf einen `ProofOfDoubleSpend` hängt von der Qualität der verfügbaren Informationen ab. Daher wird zwischen einem reinen Offline-Szenario und einem Layer-2-gestützten Szenario unterschieden.

### Szenario A: Reine Offline-Erkennung (Maximale Vorsicht)

Dies ist der Standardfall, wenn ein Wallet einen Konflikt lokal feststellt (z.B. durch Synchronisation von Fingerprints mit einem Peer), ohne ein autoritatives Urteil von einem Server zu haben.

- **Warum diese Strategie?** Ohne einen zentralen Schiedsrichter, der den genauen Zeitpunkt der Transaktionen kennt, ist es unmöglich, sicher zu bestimmen, welcher der widersprüchlichen Pfade der "legitime" ist. Die einzig sichere Annahme zum Schutz des Nutzers ist, alle Pfade, die vom Betrugspunkt (`fork_point_prev_hash`) ausgehen, als unzuverlässig zu betrachten.

- **Aktion des Wallets:** Das Wallet stellt **alle** lokalen Instanzen des Gutscheins, die von diesem Betrug betroffen sind, auf den Status `VoucherStatus::Quarantined`. Es wird keine neue Transaktion erstellt. Der Schutz ist passiv, indem die kompromittierten Gutschein-Zweige eingefroren werden.


### Szenario B: Layer-2-gestützte Lösung (Autoritatives Urteil)

Dieser Fall tritt ein, wenn das Wallet einen `ProofOfDoubleSpend` verarbeitet, der ein signiertes `Layer2Verdict` enthält.

- **Warum diese Strategie?** Das Urteil des Servers stellt eine "höhere Wahrheit" dar. Da der Server als neutrale Instanz die Fingerprints in einer global geordneten Reihenfolge empfängt, kann er autoritativ festlegen, welche Transaktion zuerst da war. Dies schafft Bestimmtheit und hält den legitimen Teil des Gutscheinwerts im Umlauf, was einen starken Anreiz zur Nutzung von Layer-2-Diensten schafft.

- **Aktion des Wallets:**

  1. Das Wallet verifiziert die Signatur des `Layer2Verdict`.

  2. Der Gutschein-Zweig, der die laut Urteil **gültige** Transaktion enthält, bleibt `Active` (oder wird aus der Quarantäne reaktiviert).

  3. Der Gutschein-Zweig, der die **ungültige** Transaktion enthält, wird auf `VoucherStatus::Quarantined` gesetzt.

  4. **Zusätzlich** wird für den ungültigen Zweig eine **Sperr-Transaktion** (`t_type: "block"`) vorbereitet. Diese wird vom Entdecker signiert und formal an den Betrüger als Empfänger adressiert (auch wenn sie nicht aktiv gesendet wird). Diese Transaktion dient als permanenter, "on-chain" dokumentierter Endpunkt, der den Entdecker davor schützt, den nun bekanntermaßen ungültigen Gutschein selbst weiterzugeben und damit einen neuen Double-Spend zu verursachen.


### Zusammenspiel und Fazit

Das System ist so konzipiert, dass es standardmäßig im Modus der maximalen Sicherheit (Offline) operiert. Sobald höherwertige, autoritative Informationen (Layer 2) verfügbar werden, schaltet es in einen Modus der Bestimmtheit und Nützlichkeit um. Der `ProofOfDoubleSpend` ist der universelle Informationsträger, der beide Zustände abbilden kann.