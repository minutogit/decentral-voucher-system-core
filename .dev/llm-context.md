...
```json
{
  ...
  "transactions": [ // Eine chronologische Liste aller Transaktionen dieses Gutscheins.
    {
      "t_id": "STRING",                 // Eindeutige ID der Transaktion.
      ...
      "sender_signature": "STRING"      // Digitale Signatur des Senders für diese Transaktion.
    }
  ],
  "additional_signatures": [ // Ein Array für zusätzliche, optionale Signaturen, die an den Gutschein angehängt werden können.
    {
      "signer_id": "STRING",            // Eindeutige ID des zusätzlichen Unterzeichners.
      "signature": "STRING",            // Die digitale Signatur.
...
* `amount`: Übertragener Betrag.
* `sender_remaining_amount`: (Relevant bei Teilzahlungen) Der Restbetrag, der beim Sender verbleibt.
* `transaction_time`: Zeitstempel der Transaktion.
* `sender_signature`: Digitale Signatur des Senders zur Sicherstellung der Integrität.

### Double-Spending-Erkennung (Basis-Layer)
* Erkennung durch Prüfung, ob mehrere Transaktionen desselben Senders denselben `previous_hash` teilen.
